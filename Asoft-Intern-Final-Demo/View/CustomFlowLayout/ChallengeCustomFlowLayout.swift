...
class ChallengeCustomFlowLayout: UICollectionViewFlowLayout {
    ...
    override func prepare() {
        ...
        self.itemSize = CGSize(width: (self.collectionView?.frame.size.height)!, height: (self.collectionView?.frame.size.height)!)
        let inset = ((self.collectionView?.bounds.width)! - (self.collectionView?.bounds.height)!)*0.5
    ...
    override func layoutAttributesForElements(in rect: CGRect) -> [UICollectionViewLayoutAttributes]? {
        ...
        for attrs in array! {
            let delta = abs(centerX - attrs.center.x)*0.45
